...
public class JudgeDto {
    ...
    public void judge(Long submissionId) throws ApiException {
        ...
        submissionApi.updateVerdict(submissionId, verdict, runtimeMs);
        log.info("Judged submission {} -> {} ({} ms)", submissionId, verdict, runtimeMs);
        ...
        if (Objects.nonNull(matchId)) {
            ...
            matchEventPublisher.publish(matchId, ConversionHelper.toSubmissionJudgedEvent(submission, verdict));
            ...
            if (verdict == Verdict.ACCEPTED && matchApi.finish(matchId, submission.getUserId())) {
