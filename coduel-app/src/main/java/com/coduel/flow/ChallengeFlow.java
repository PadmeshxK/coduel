...
public class ChallengeFlow {
    ...
    public ChallengeResult create(String challengerGoogleId, Long targetUserId, String problemSlug) throws ApiException {
        ...
        // A shared-problem challenge names a specific problem — validate it exists up front.
        if (problemSlug != null && !problemSlug.isBlank()) {
            problemApi.getCheckBySlug(problemSlug);
        }
    ...
    public ChallengeResult accept(Long challengerUserId, String acceptorGoogleId, String problemSlug) throws ApiException {
        User accepter = userApi.getCheckByGoogleId(acceptorGoogleId);
        User challenger = userApi.getCheckById(challengerUserId);
        // Duel on the shared problem if one was named, otherwise a random one (plain challenge).
        Problem problem = (problemSlug != null && !problemSlug.isBlank())
                ? problemApi.getCheckBySlug(problemSlug)
                : problemApi.getCheckRandomProblem();
